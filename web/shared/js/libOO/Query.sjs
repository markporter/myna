	...
	*/
	Myna.Query = function (optionsOrResultSet){
		...
		*/
		this.log =false;
		/* Property: db
			the <Myna.Database> object associated with this query
			Detail:
...
/* Function: getRowByColumn
		returns first row of the result set that matches the column and value supplied
		Parameters:
		...
		Returns:
			first row of the result set that matches the column and value supplied
		*/
			...
			if (sqlParameters instanceof Myna.QueryParams){
				sqlParameters.params.forEach(function(element,index){
					try {
						...
						e.message +="<br>attempting to set value '" +
							String(element.value) + "' to a parameter of type '"+
							element.type +"' typeName: '" + element.typeName + "'.";
						throw e;
					}
				});
...
/* Constructor: QueryResultRow
	...
	Parameters:
		rs				-	a java.sql.ResultSet object, initialized to a valid
							row.
		query			-	the <Myna.Query> object that created the row. This
							object is expects to be initialized with the query
							metadata prior to constructing this object
	*/
		...
		*/
		getValue:function(indexOrName){
			...
			switch (col.typeId){
				...
				case Types.DATE:
					curValue = this.rs.getTimestamp(colIndex+1);
					if(curValue !== null) {
						var d =new Date();
						d.setMonth(curValue.getMonth());
						d.setDate(curValue.getDate());
						d.setYear(curValue.getYear() + 1900);
						d.setHours(curValue.getHours());
						d.setMinutes(curValue.getMinutes());
						d.setSeconds(curValue.getSeconds());
						d.setMilliseconds(curValue.getNanos()/1e6);
						curValue = d;
...
/* Class: Myna.QueryParams
	...
	Detail:
	This object can be constructed from an array or via the
